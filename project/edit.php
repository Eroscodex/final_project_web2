...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name != '' && $price != '' && $stock != '') {
        ...
        if ($stmt->execute()) {
            header("Location: index.php?msg=Product+Updated");
            exit;
        } else {
...
echo "    <title>Edit Product - Neon Store</title>";
...
echo "                <input type='text' name='product_name' value='" . htmlspecialchars($data['product_name']) . "' required>";
echo "            </div>";
echo "            <div class='form-group'>";
echo "                <label>Price (₱)</label>";
echo "                <input type='number' step='0.01' name='price' value='" . $data['price'] . "' required>";
echo "            </div>";
echo "            <div class='form-group'>";
echo "                <label>Stock</label>";
echo "                <input type='number' name='stock' value='" . $data['stock'] . "' required>";
echo "            </div>";
